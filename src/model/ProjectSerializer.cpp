                                       ...
                                       juce::AudioFormatManager& formatManager)
{
    ...
    session.setTimeSignature(
        json.getProperty("timeSigNumerator", 4),
        ...
        for (int i = 0; i < tracksVar.size(); ++i)
        {
            ...
            if (clipsVar.isArray())
            {
                for (int j = 0; j < clipsVar.size(); ++j)
                {
                    ...
                    clip.gainDb = static_cast<float>(
                        (double)clipVar.getProperty("gainDb", 0.0));
...
juce::var ProjectSerializer::clipToJson(const Clip& clip, const juce::File& projectDir)
{
    ...
    if (clip.asset)
        obj->setProperty("assetPath", makeRelativePath(juce::File(clip.asset->filePath), projectDir));
