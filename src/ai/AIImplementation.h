...
public:
    ...
    void provideFeedback(const PlanningRequest& request,
...
public:
    ...
    std::vector<MixAssistant::MixSuggestion> getSuggestionsForTrack(
...
private:
    ...
    float predictUserPreference(const std::string& trackId,
...
public:
    ...
    std::vector<TransientDetector::Transient> detectTransients(const float* audio,
                                                               int numSamples,
...
private:
    ...
    std::vector<float> calculateSpectralFlux(const float* audio, int numSamples,
                                            int windowSize, int hopSize) const;
    std::vector<float> calculateEnergy(const float* audio, int numSamples,
                                      int windowSize, int hopSize) const;
    std::vector<float> calculateHighFrequencyContent(const float* audio, int numSamples,
                                                     int windowSize, int hopSize) const;

    // Peak picking
    std::vector<int> pickPeaks(const std::vector<float>& signal,
