    ...
    while (heartbeatRunning_) {
        ...
        if (ipc_ && !ipc_->checkHeartbeat()) {
            ...
            if (process_) {
                // Mark as crashed
                crashed_ = true;
                crashLog_ = "Sandbox process heartbeat timeout";
            }
        }
    }
}
...
void SandboxServer::handleLoadPlugin(const SandboxIPC::Message& message) {
    try {
        ...
        auto instance = manager.createInstance(pluginId);
        if (!instance) {
            sendError("Failed to create plugin instance");
            return;
        }
        
        if (!instance->initialize(44100.0, 512)) {
            sendError("Failed to initialize plugin");
            return;
        }
        
        currentPlugin_ = std::move(instance);
...
void SandboxServer::handleGetParameter(const SandboxIPC::Message& message) {
    if (!currentPlugin_) {
        sendError("No plugin loaded");
        return;
    }
    ...
    try {
        float value = currentPlugin_->getParameterValue(message.parameterId);
        
        SandboxIPC::Message response(SandboxIPC::MessageType::Heartbeat);
        response.parameterValue = value;
        sendResponse(response);
        
    } catch (const std::exception& e) {
        sendError(std::string("Get parameter error: ") + e.what());
    }
}

void SandboxServer::handleSaveState(const SandboxIPC::Message& message) {
    if (!currentPlugin_) {
        sendError("No plugin loaded");
        return;
    }
    ...
    try {
        PluginState state = currentPlugin_->getState();
        
        SandboxIPC::Message response(SandboxIPC::MessageType::Heartbeat);
        
        // Convert state to string (simplified)
        response.data = "state_data_placeholder"; // In reality, serialize properly
        
        sendResponse(response);
        
    } catch (const std::exception& e) {
        sendError(std::string("Save state error: ") + e.what());
    }
}

void SandboxServer::handleLoadState(const SandboxIPC::Message& message) {
    if (!currentPlugin_) {
        sendError("No plugin loaded");
        return;
    }
    ...
    try {
        PluginState state;
        state.pluginId = currentPlugin_->getPluginInfo().id;
        state.usesChunk = false;
        
        // In reality, deserialize message.data into state
        
        currentPlugin_->setState(state);
        
        SandboxIPC::Message response(SandboxIPC::MessageType::Heartbeat);
        sendResponse(response);
        
    } catch (const std::exception& e) {
        sendError(std::string("Load state error: ") + e.what());
    }
}
