...
void PianoSynth::noteOn(int noteNumber, float velocity)
{
    ...
    if (it != activeNotes_.end())
    {
        ...
        Voice* voice = &voices_[static_cast<size_t>(it->second)];
    ...
    if (!voice)
    {
        ...
        {
            if (voices_[static_cast<size_t>(voiceIdx)].envelopePhase < voice->envelopePhase)
            {
                voice = &voices_[static_cast<size_t>(voiceIdx)];
...
void PianoSynth::render(float* leftOut, float* rightOut, int numSamples)
{
    ...
    // Clear output
    if (leftOut) std::memset(leftOut, 0, static_cast<size_t>(numSamples) * sizeof(float));
    if (rightOut) std::memset(rightOut, 0, static_cast<size_t>(numSamples) * sizeof(float));
...
bool PianoSynth::loadPianoSample(const juce::File& sampleFile)
{
    ...
    pianoSample_.resize(static_cast<size_t>(reader->lengthInSamples));

    if (reader->numChannels == 1)
    {
        float* channelPtr = pianoSample_.data();
        reader->read(&channelPtr, 1, 0, static_cast<int>(reader->lengthInSamples));
    }
    else
    {
        // Mix down to mono
        std::vector<float> tempBuffer(static_cast<size_t>(reader->lengthInSamples * reader->numChannels));
        std::vector<float*> channelPtrs;
        for (int ch = 0; ch < reader->numChannels; ++ch)
            channelPtrs.push_back(&tempBuffer[static_cast<size_t>(ch * reader->lengthInSamples)]);

        reader->read(channelPtrs.data(), reader->numChannels, 0, static_cast<int>(reader->lengthInSamples));
        ...
        for (int64_t i = 0; i < reader->lengthInSamples; ++i)
        {
            ...
            for (int ch = 0; ch < reader->numChannels; ++ch)
            {
                sum += tempBuffer[static_cast<size_t>(i * reader->numChannels + ch)];
            }
            pianoSample_[static_cast<size_t>(i)] = sum / reader->numChannels;
        }
    }
...
void PianoSynth::generateDefaultPianoSample()
{
    ...
    for (int i = 0; i < sampleLength; ++i)
    {
        ...
        pianoSample_[static_cast<size_t>(i)] = sample * envelope;
    }
...
{
    auto it = activeNotes_.find(noteNumber);
    return (it != activeNotes_.end()) ? &voices_[static_cast<size_t>(it->second)] : nullptr;
}
...
void PianoSynth::startVoice(Voice* voice, int noteNumber, float velocity)
{
    ...
    voice->phaseInc = voice->pitchRatio * (voice->sampleRate / 44100.0f);
...
float PianoSynth::getNextSample(Voice& voice)
{
    ...
    float sample2 = (static_cast<size_t>(index + 1) < static_cast<size_t>(voice.sampleLength)) ? voice.sampleData[index + 1] : 0.0f;
...
float PianoSynth::calculateEnvelope(Voice& voice)
{
    ...
    switch (voice.envelopePhase)
    {
        case Attack:
        {
            ...
            voice.envelope = voice.envelopeTime / attackSamples;
            voice.envelopeTime += 1.0f;
        ...
        case Decay:
        {
            ...
            voice.envelope = 1.0f - (1.0f - sustainLevel) * (voice.envelopeTime / decaySamples);
            voice.envelopeTime += 1.0f;
            if (voice.envelopeTime >= decaySamples)
            {
        ...
        case Release:
        {
            ...
            voice.envelope = sustainLevel * (1.0f - voice.envelopeTime / releaseSamples);
            voice.envelopeTime += 1.0f;
            if (voice.envelopeTime >= releaseSamples)
            {
                voice.envelope = 0.0f;
