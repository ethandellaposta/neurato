...
PluginInfo PluginScanner::createPluginInfo(const juce::PluginDescription& desc) {
    ...
    info.supportsMidi = desc.isMidiEffect;
...
PluginManager::PluginManager() : scanner_(std::make_unique<PluginScanner>()) {
}
...
void PluginManager::scanPlugins(const std::vector<std::string>& searchPaths) {
    if (!scanner_) return;

    // Set up progress callback
    scanner_->setProgressCallback([this](int current, int total, const std::string& message) {
        juce::Logger::writeToLog("Scanning plugins: " + std::to_string(current) +
                                 "/" + std::to_string(total) + " " + message);
    });

    // Scan VST3 plugins
    auto vst3Future = scanner_->scanVST3(searchPaths);
    auto vst3Plugins = vst3Future.get();

    // Scan AU plugins (macOS only)
#ifdef JUCE_MAC
    auto auFuture = scanner_->scanAU(searchPaths);
    auto auPlugins = auFuture.get();

    for (const auto& plugin : auPlugins) {
        addPlugin(plugin);
    }
#endif
...
void PluginManager::addPlugin(const PluginInfo& info) {
    std::lock_guard<std::mutex> lock(databaseMutex_);
    pluginDatabase_[info.id] = info;
}

void PluginManager::removePlugin(const std::string& pluginId) {
    std::lock_guard<std::mutex> lock(databaseMutex_);
    pluginDatabase_.erase(pluginId);
}
...
std::vector<PluginInfo> PluginManager::getAllPlugins() const {
    ...
    std::vector<PluginInfo> plugins;
    for (const auto& pair : pluginDatabase_) {
...
std::vector<PluginInfo> PluginManager::getInstruments() const {
    std::lock_guard<std::mutex> lock(databaseMutex_);

    std::vector<PluginInfo> plugins;
    for (const auto& pair : pluginDatabase_) {
        if (pair.second.isInstrument) {
            plugins.push_back(pair.second);
        }
    }
...
std::vector<PluginInfo> PluginManager::getEffects() const {
    std::lock_guard<std::mutex> lock(databaseMutex_);

    std::vector<PluginInfo> plugins;
    for (const auto& pair : pluginDatabase_) {
        if (pair.second.isEffect) {
            plugins.push_back(pair.second);
        }
    }
...
PluginInfo PluginManager::getPluginInfo(const std::string& pluginId) const {
    ...
    auto it = pluginDatabase_.find(pluginId);
    return (it != pluginDatabase_.end()) ? it->second : PluginInfo{};
}

std::unique_ptr<PluginInstance> PluginManager::createInstance(const std::string& pluginId) {
    auto info = getPluginInfo(pluginId);
    if (info.id.empty()) {
        return nullptr;
    }

    auto instance = std::make_unique<PluginInstance>(info);
    return instance;
}

void PluginManager::loadPluginDatabase() {
    std::lock_guard<std::mutex> lock(databaseMutex_);
...
void PluginManager::savePluginDatabase() {
    std::lock_guard<std::mutex> lock(databaseMutex_);
    ...
    for (const auto& pair : pluginDatabase_) {
        ...
        file << "Plugin: " << plugin.name << std::endl;
        file << "  ID: " << plugin.id << std::endl;
        file << "  Manufacturer: " << plugin.manufacturer << std::endl;
        file << "  Format: " << static_cast<int>(plugin.format) << std::endl;
        file << "  Path: " << plugin.filePath << std::endl;
        file << std::endl;
    }
}

void PluginManager::clearDatabase() {
    std::lock_guard<std::mutex> lock(databaseMutex_);
    pluginDatabase_.clear();
}

std::string PluginManager::getDatabasePath() const {
    ...
    return appDataDir.getChildFile("plugins.txt").getFullPathName().toStdString();
}

// PluginNode implementation
PluginNode::PluginNode(const std::string& id, std::unique_ptr<PluginInstance> instance)
    : AudioNode(Type::Plugin, id), plugin_(std::move(instance)) {

    if (plugin_) {
        const auto& info = plugin_->getPluginInfo();
        setInputChannelCount(info.numInputChannels);
        setOutputChannelCount(info.numOutputChannels);
    }
}

PluginNode::~PluginNode() = default;

void PluginNode::process(AudioBuffer& input, AudioBuffer& output,
                        int numSamples, SampleCount position) noexcept {
    if (isBypassed() || !plugin_ || !plugin_->isInitialized()) {
    ...
    // Convert input buffer to JUCE format
    convertAudioBuffer(input, juceBuffer_);

    // Clear any existing MIDI messages
    midiBuffer_.clear();

    // Process through plugin
    plugin_->processAudio(juceBuffer_, midiBuffer_);

    // Convert output back to our format
    convertAudioBuffer(juceBuffer_, output);
}

std::vector<ParameterInfo> PluginNode::getParameters() const {
    if (!plugin_) return {};

    auto pluginParams = plugin_->getParameters();
    std::vector<ParameterInfo> params;

    for (const auto& pp : pluginParams) {
        ParameterInfo info;
        info.id = pp.id;
        info.name = pp.name;
        info.minValue = pp.minValue;
        info.maxValue = pp.maxValue;
        info.defaultValue = pp.defaultValue;
        info.isAutomatable = pp.isAutomatable;
        info.unit = pp.unit;
...
float PluginNode::getParameterValue(const std::string& paramId) const {
    return plugin_ ? plugin_->getParameterValue(paramId) : 0.0f;
}

void PluginNode::setParameterValue(const std::string& paramId, float value) {
    if (plugin_) {
        plugin_->setParameterValue(paramId, value);
    }
}

void PluginNode::prepareToPlay(double sampleRate, int samplesPerBlock) {
    if (plugin_) {
        plugin_->initialize(sampleRate, samplesPerBlock);
    }

    // Prepare JUCE buffer
    juceBuffer_.setSize(getOutputChannelCount(), samplesPerBlock);
    channelPointers_.resize(getOutputChannelCount());

    for (int ch = 0; ch < getOutputChannelCount(); ++ch) {
        channelPointers_[ch] = juceBuffer_.getWritePointer(ch);
    }
}

void PluginNode::reset() {
    if (plugin_) {
        plugin_->resetToDefaults();
    }
}

const PluginInfo& PluginNode::getPluginInfo() const {
    return plugin_ ? plugin_->getPluginInfo() : PluginInfo{};
}

void PluginNode::setState(const PluginState& state) {
    if (plugin_) {
        plugin_->setState(state);
    }
}

PluginState PluginNode::getState() const {
    return plugin_ ? plugin_->getState() : PluginState{};
}

void PluginNode::convertAudioBuffer(const AudioBuffer& input, juce::AudioBuffer<float>& output) {
    output.setSize(input.numChannels, input.numSamples, false, false, true);
...
void PluginNode::convertAudioBuffer(const juce::AudioBuffer<float>& input, AudioBuffer& output) {
    // This is a simplified conversion - in practice, you'd need to handle
    // the channel pointer setup properly
    for (int ch = 0; ch < std::min(input.getNumChannels(), output.numChannels); ++ch) {
...
// PluginAutomationController implementation
PluginAutomationController::PluginAutomationController(PluginNode* node)
    : pluginNode_(node) {
}

void PluginAutomationController::addAutomationLane(const std::string& paramId,
                                                  std::shared_ptr<AutomationLane> lane) {
    automationLanes_[paramId] = lane;
}

void PluginAutomationController::removeAutomationLane(const std::string& paramId) {
    automationLanes_.erase(paramId);
}

void PluginAutomationController::processWithAutomation(AudioBuffer& input, AudioBuffer& output,
                                                     int numSamples, SampleCount position) {
    // Update parameters from automation
    for (const auto& pair : automationLanes_) {
        updateParameterFromAutomation(pair.first, position);
    }

    // Process the plugin
    if (pluginNode_) {
        pluginNode_->process(input, output, numSamples, position);
    }
}

std::string PluginAutomationController::mapParameterId(const std::string& pluginParamId) const {
    auto it = parameterIdMap_.find(pluginParamId);
    return (it != parameterIdMap_.end()) ? it->second : pluginParamId;
}

std::string PluginAutomationController::unmapParameterId(const std::string& automationParamId) const {
    for (const auto& pair : parameterIdMap_) {
        if (pair.second == automationParamId) {
            return pair.first;
        }
    }
    return automationParamId;
}

void PluginAutomationController::updateParameterFromAutomation(const std::string& paramId,
                                                               SampleCount position) {
    auto it = automationLanes_.find(paramId);
    if (it != automationLanes_.end() && it->second && pluginNode_) {
        float value = it->second->getValueAt(position);
        pluginNode_->setParameterValue(paramId, value);
    }
}
