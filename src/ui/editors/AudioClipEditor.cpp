...
{
    closeBtn_.setColour(juce::TextButton::buttonColourId, juce::Colour(ampl::Theme::bgElevated));
...
void AudioClipEditor::paint(juce::Graphics& g)
{
    ...
    if (clip && clip->asset)
    {
        g.setColour(juce::Colour(ampl::Theme::textSecondary));
        g.setFont(ampl::Theme::bodyFont());
...
void AudioClipEditor::paintWaveform(juce::Graphics& g, juce::Rectangle<int> area)
{
    ...
    // Background
    g.setColour(juce::Colour(ampl::Theme::bgDeepest));
    g.fillRect(area);
    ...
    g.setColour(juce::Colour(ampl::Theme::accentOrange).withAlpha(0.6f));
