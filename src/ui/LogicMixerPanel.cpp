...
void LogicMixerPanel::drawBackground(juce::Graphics& g) {
    ...
    g.fillAll(Theme::colors.backgroundDark);
...
LogicMixerPanel::ChannelStripComponent::ChannelStripComponent(
    ...
    recordArmButton_->setColour(juce::TextButton::buttonColourId, strip_.recordArm ?
                               Theme::colors.warning : Theme::colors.surface);
    ...
    for (int i = 0; i < LogicMixerChannel::kPluginSlots; ++i) {
        ...
        button->setColour(juce::TextButton::buttonColourId, Theme::colors.surface);
    ...
    nameLabel_->setColour(juce::Label::textColourId, Theme::colors.textPrimary);
    ...
    for (int i = 0; i < LogicMixerChannel::kPluginSlots; ++i) {
        if (!strip_.pluginChain[i].empty()) {
            ...
            if (strip_.pluginBypass[i]) {
                pluginButtons_[i]->setColour(juce::TextButton::buttonColourId, Theme::colors.surface);
            } else {
                pluginButtons_[i]->setColour(juce::TextButton::buttonColourId, Theme::colors.accent);
            }
        } else {
    ...
    for (int i = 0; i < 8; ++i) {
        ...
        if (!strip_.sendTargets[i].empty()) {
            ...
            sendButtons_[i]->setColour(juce::TextButton::buttonColourId,
                                       strip_.sendPreFader[i] ? Theme::colors.accent : Theme::colors.surface);
        } else {
            sendButtons_[i]->setButtonText("S" + std::to_string(i + 1));
            sendButtons_[i]->setColour(juce::TextButton::buttonColourId, Theme::colors.surface);
        }
    }
}
...
void LogicMixerPanel::ChannelStripComponent::drawChannelBackground(juce::Graphics& g) {
    ...
    if (strip_.solo) {
        g.setColour(Theme::colors.warning.withAlpha(0.2f));
        g.fillRect(bounds);
    }
...
void LogicMixerPanel::ChannelStripComponent::drawMeters(juce::Graphics& g) {
    ...
    // Draw peak indicator
    g.setColour(Theme::colors.warning);
    int peakHeight = 5;
...
LogicMixerPanel::MasterSection::MasterSection(LogicMixerPanel& parent) : parent_(parent) {
    ...
    masterMuteButton_->setColour(juce::TextButton::buttonColourId, Theme::colors.surface);
...
void LogicMixerPanel::MasterSection::updateMasterBus(const LogicEnvironment::Bus& bus) {
    ...
    masterMuteButton_->setColour(juce::TextButton::buttonColourId,
                                 bus.mute ? Theme::colors.accent : Theme::colors.surface);
    repaint();
}
...
void LogicMixerPanel::MasterSection::paint(juce::Graphics& g) {
    ...
    g.fillAll(Theme::colors.backgroundDarker);

    // Draw "MASTER" label
    g.setColour(Theme::colors.textPrimary);
    g.setFont(Theme::fonts.label);
